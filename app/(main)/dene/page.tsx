...
export default function DenePage() {
  ...
  const { photo, setPhoto } = useStudio();
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 720, marginBottom: 48 }}>
        ...
        >
          Yüzün net görünen, ön cepheden, saçın toplanmış bir fotoğraf en iyi sonucu verir. Kamera
          kullanılmaz — dosyan sadece deneme sırasında işlenir.
        </p>
      </div>
      ...
      >
        ...
        <div>
          ...
          >
            {samples.map((s) => {
              const active = photo?.kind === "sample" && photo.seed === s.seed;
              return (
                <button
                  key={s.seed}
                  onClick={() => setPhoto({ kind: "sample", seed: s.seed })}
                  style={{
                    borderRadius: "var(--radius-md)",
                    overflow: "hidden",
                    outline: active ? "2px solid var(--green)" : "1px solid var(--line)",
